...
def ml_dnn_ssea(station_name="종로구"):
    ...
    for target in targets:
        ...
        output_dir = Path("/mnt/data/dnn_stl_sea/" + station_name + "/"+ target + "/")
...
class BaseDNNModel(LightningModule):
    ...
    def forward_trend(self, xts, order=(1, 0, 1)):
        yts = []
        for xt in xts:
            #model = ARIMA(xt, order=order)
            model = SimpleExpSmoothing(xt)
            model_fit = model.fit()
            #yt, err_arr, conf_ints = model_fit.forecast(steps=self.hparams.output_size)
            yt = model_fit.forecast(steps=self.hparams.output_size)
            yts.append(yt.astype('float32'))

        return torch.as_tensor(tuple(yts)).to(device)
    ...
    def training_step(self, batch, batch_idx):
        x, xt, y, dates = batch
        y_hat = self(x)
        yt_hat = self.forward_trend(xt)
        _loss = self.loss(y_hat + yt_hat, y)
    ...
    def prepare_data(self):
        ...
        train_valid_set = data.DNNSTLSeasonalityDataset(
        ...
        test_set = data.DNNSTLSeasonalityDataset(
            station_name=self.station_name,
            target=self.target,
            filepath="/input/python/input_jongro_imputed_hourly_pandas.csv",
            features=self.features,
    ...
    def collate_fn(self, batch):
        ...
        xs, xts, ys, dates = zip(*batch)

        return torch.as_tensor(xs), xts, torch.as_tensor(ys), dates
