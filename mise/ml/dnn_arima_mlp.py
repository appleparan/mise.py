...
def ml_dnn_arima_mlp(station_name="종로구"):
    ...
    for target in targets:
        ...
        output_dir = Path("/mnt/data/dnn_arima_mlp/" +
                          station_name + "/" + target + "/")
...
class BaseDNNModel(LightningModule):
    ...
    def forward(self, x):
        # vectorize
        x = x.contiguous().view(-1, self.hparams.input_size).to(device)
    ...
    def prepare_data(self):
        ...
        train_valid_set = data.DNNARIMAMLPDataset(
...
def plot_scatter(hparams, df_obs, df_sim, output_dir, fname_prefix):
    ...
    for t in range(hparams.output_size):
        plt_fname = output_dir / \
            (fname_prefix + "_h" + str(t).zfill(2) + ".png")
