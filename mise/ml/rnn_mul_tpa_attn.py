...
class BaseTPAAttnModel(LightningModule):
    ...
    def setup(self, stage=None):
        ...
        * TODO : Refactoring https://pytorch-lightning.readthedocs.io/en/stable/datamodules.html
        ...
        train_set = MultivariateRNNDataset(
        ...
        # train_set.plot_pdf(self.png_dir / "dist", self.svg_dir / "dist", "train")
        # save train set
        train_set.to_csv(
            self.data_dir / ("df_train_set_" + self.target + ".csv"))

        valid_set = MultivariateRNNDataset(
            station_name=self.station_name,
            target=self.target,
            filepath="/input/python/input_jongno_imputed_hourly_pandas.csv",
            features=self.features,
        ...
        # valid_set.plot_pdf(self.png_dir / "dist", self.svg_dir / "dist", "valid")
        ...
        test_set = MultivariateRNNDataset(
            ...
            scaler_X=train_set.scaler_X,
            scaler_Y=train_set.scaler_Y)
        test_set.to_csv(self.data_dir / ("df_test_set_" + self.target + ".csv"))

        test_set.transform()

        # test_set.plot_pdf(self.png_dir / "dist", self.svg_dir / "dist", "test")
