...
import statsmodels.tsa.statespace.sarimax as sarm
...
def stats_sarima(station_name="종로구"):
    ...
    targets = ["PM10", "PM25"]
    output_size = 24
    train_fdate = dt.datetime(2017, 7, 1, 0).astimezone(seoultz)
    ...
    for target in targets:
        dir_prefix = Path("/mnt/data/sarima/" +
                          station_name + "/" + target + "/")
...
def sim_sarima(_df_train, _df_test, target, order, fdate, tdate, norm_maxlog, output_size):
    ...
    # daily seasonality
    sorder = (order[0], order[1], order[2], 24)

    for i, (index, row) in tqdm.tqdm(enumerate(df_test.iterrows()), total=len(dates)-1):
        if i > len(dates) - 1:
            break

        model = sarm.SARIMAX(endog[-sz:], order=order, seasonal_order=sorder)
...
def plot_sarima(df_sim, df_obs, target, order, _test_fdate, _test_tdate, station_name, output_size):
    dir_prefix = Path("/mnt/data/sarima/" + station_name + "/" + target + "/")
    ...
    corr_fname = "corr_sarima(" + \
        str(order[0]) + ", " + str(order[1]) + ", " + str(order[2]) + ")_" + \
        target + "_" + str(t).zfill(2) + ".png"
    csv_fname = "corr_hourly_sarima(" + \
        str(order[0]) + ", " + str(order[1]) + ", " + str(order[2]) + ")_" + \
        target + "_h" + str(t).zfill(2) + ".csv"
